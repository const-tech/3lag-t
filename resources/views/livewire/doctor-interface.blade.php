        ...
        <div class="col-lg-3 col-xl-2 ps-0" wire:poll.30000ms>
            ...
            @if ($patient && $patient->packages->count() > 0)
                @foreach ($patient->packages as $item)
                    @if ($item->diagnoses->count() < $item->dayes_period)
                        <div class="box-plan">
                            <button wire:click="selectPackage({{ $item->id }})"
                                class="name btn">{{ $item->package->title }}</button>
                        </div>
                    @endif
                @endforeach
                ...
                <div class="d-flex align-items-center gap-1 justify-content-between">
                    ...
                    @if ($patient)
                        <a target="_blank" class="btn btn-sm btn-info"
                            href="{{ route('doctor.invoices.index', ['patient' => $patient->id]) }}">فواتير المريض</a>
                    @endif
                </div>
                {{-- @if ($patient)
